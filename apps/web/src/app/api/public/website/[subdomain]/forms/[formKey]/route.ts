...
type RouteContext = {
  params: Promise<{ subdomain: string; formKey: string }>;
};
...
export async function POST(request: Request, context: RouteContext) {
  const { subdomain, formKey } = await context.params;
  ...
  for (const candidate of candidates) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/public/website/${encodeURIComponent(candidate)}/forms/${encodeURIComponent(formKey)}/submit`,
        {
          method: "POST",
          headers: {
      ...
      return NextResponse.json(
        { message: payload || "Unable to submit form." },
        { status: response.status },
      );
    } catch {
      // try next
  ...
  return NextResponse.json({ message: "Unable to submit form." }, { status: 404 });
}
